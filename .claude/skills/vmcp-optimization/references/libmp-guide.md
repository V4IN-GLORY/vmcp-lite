# LibMP guide (verbatim from github.com/Roblox/libmp/docs/SKILL.md)

Concepts, agent notes, and the two reference iterators. The API tables are in `libmp-api.md`; the scope catalogue is in `scopes/`. `execute_luau` in this text means VMCP's `run_luau`; `require("@rbx/LibMP")` doesn't exist here -- use `vmcp.Profile.LibMP()`.
