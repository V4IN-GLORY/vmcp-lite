import type { Surface } from "./image.js";

/**
 * A 5x7 bitmap font, the only text either renderer has. Uppercase, digits and the punctuation a
 * HUD readout uses; anything else draws as a blank. Good enough to read a label, not a font.
 */
...
/** Draws `text` with its top-left at (left, top), each glyph pixel `scale` wide. */
export function label(
	...
	top: number,
	scale: number,
	color: [number, number, number] = [235, 235, 240],
	alpha = 1,
			...
			for (let column = 0; column < 5; column++) {
				...
				for (let dy = 0; dy < scale; dy++) {
					for (let dx = 0; dx < scale; dx++) {
						...
						surface.blend(px, py, color[0], color[1], color[2], alpha);
					}
				}
			}
		}
